...
(async () => {
  ...
  assert.ok(Array.isArray(imported.analysis.subtitleCues), 'Import always returns a subtitle timeline payload');
  assert.equal((await fetch(base + imported.sourceUrl)).status, 200);
  assert.ok(imported.analysis.segments.length >= 3, 'Imported video exposes multiple timeline segments');
  for (const segment of imported.analysis.segments) {
    assert.ok(segment.thumbnailUrl, 'Every timeline segment has a representative frame');
    const thumbnail = await fetch(base + segment.thumbnailUrl); assert.equal(thumbnail.status, 200);
  ...
  assert.ok(saved.plan.ready); assert.ok(saved.plan.items.length >= 2, 'Natural-language direction returns a structured plan');
  const direction = await json('/api/direction', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobId: imported.jobId, prompt: 'Opening: Start here', draft: { hook: 'Start here', subject: 'Main idea', cta: 'Try it today' } }) });
  assert.equal(direction.draft.hook, 'Start here');
  ...
  assert.deepEqual(persisted.creativePlan, saved.plan);
  assert.equal(persisted.directionConversation.length, 1);
  ...
  console.log(JSON.stringify({ ok: true, jobId: imported.jobId, outputUrl: rendered.outputUrl, segmentCount: imported.analysis.segments.length, subtitleState: imported.analysis.subtitleState, subtitleCount: imported.analysis.subtitleCues.length, checks: ['assets', 'video upload', 'metadata', 'storyboard detection', 'segment thumbnails', 'separate extracted-subtitle panel', 'structured creative plan', 'natural-language direction conversation', 'reference file persistence', 'notes and clearing notes', 'Spanish text MP4 export', 'invalid URL rejection'] }, null, 2));
